...
const CATEGORIES = {
  expense: [
    { id: 'makan', label: 'Konsumsi', icon: 'fa-solid fa-utensils' },
    { id: 'bensin', label: 'Bahan Bakar', icon: 'fa-solid fa-gas-pump' },
    { id: 'parkir', label: 'Parkir', icon: 'fa-solid fa-square-parking' },
    { id: 'belanja', label: 'Belanja', icon: 'fa-solid fa-bag-shopping' },
    { id: 'wifi', label: 'Internet', icon: 'fa-solid fa-wifi' },
    { id: 'listrik', label: 'Listrik', icon: 'fa-solid fa-bolt' },
    { id: 'transportasi', label: 'Transportasi', icon: 'fa-solid fa-motorcycle' },
    { id: 'kesehatan', label: 'Kesehatan', icon: 'fa-solid fa-heart-pulse' },
    { id: 'hiburan', label: 'Hiburan', icon: 'fa-solid fa-film' },
    { id: 'kopi', label: 'Gaya Hidup', icon: 'fa-solid fa-mug-hot' },
    { id: 'laundry', label: 'Layanan', icon: 'fa-solid fa-shirt' },
    { id: 'lainnya', label: 'Lain-lain', icon: 'fa-solid fa-ellipsis' }
  ],
  income: [
    { id: 'gaji', label: 'Pendapatan Utama', icon: 'fa-solid fa-money-bill-wave' },
    { id: 'freelance', label: 'Lepas Waktu', icon: 'fa-solid fa-laptop' },
    { id: 'bonus', label: 'Insentif', icon: 'fa-solid fa-gift' },
    { id: 'cashback', label: 'Pengembalian', icon: 'fa-solid fa-rotate-left' },
    { id: 'lainnya', label: 'Lain-lain', icon: 'fa-solid fa-ellipsis' }
  ],
  transfer: []
};
...
function getWalletName(key) {
  ...
  return (meta[key] && meta[key].name) ? meta[key].name : (key === 'cash' ? 'Tunai' : (key || '').toUpperCase());
}
...
function computeBurnoutPrediction() {
  ...
  if (daysLeft <= 0) return 'Defisit Aktif';
...
function renderHealthScore() {
  ...
  if (breakEl) {
    breakEl.innerHTML = `
      <div class="hs-metric"><div class="hs-metric-val">${Math.round(savingRate * 100)}%</div><div class="hs-metric-lbl">Retensi Dana</div></div>
      <div class="hs-metric"><div class="hs-metric-val">${budgetScore}/25</div><div class="hs-metric-lbl">Disiplin Batas</div></div>
      <div class="hs-metric"><div class="hs-metric-val">${consistencyScore}/20</div><div class="hs-metric-lbl">Konsistensi</div></div>
    `;
  }
}
...
function renderDashGoals() {
  ...
  if (!el) return;
  if (!goals.length) {
    el.innerHTML = '<div style="font-size:13px;color:var(--text-muted);padding:8px 0">Data proyektif belum didefinisikan.</div>';
    return;
  }
  el.innerHTML = goals.slice(0, 3).map(g => {
    const pct = g.target > 0 ? Math.min(100, Math.round((g.current / g.target) * 100)) : 0;
    return `
      <div class="goal-card" style="padding:12px;margin-bottom:8px">
        <div class="goal-header">
          <div class="goal-name">${g.name}</div>
          <div class="goal-pct">${pct}%</div>
        </div>
        <div class="progress-bar-wrap" style="height:6px;margin-bottom:6px"><div class="progress-bar" style="width:${pct}%"></div></div>
        <div style="display:flex;justify-content:space-between;font-size:12px;color:var(--text-secondary)"><span>${formatRp(g.current, true)}</span><span>${formatRp(g.target, true)}</span></div>
      </div>
    `;
  }).join('');
}
...
function renderDashRecurring() {
  ...
  if (!recurrings.length) {
    el.innerHTML = '<div style="font-size:13px;color:var(--text-muted);padding:8px 0">Data referensi tidak ditemukan.</div>';
    return;
  }
  ...
  el.innerHTML = upcoming.map(r => {
    ...
    const label = isOverdue ? 'Jatuh Tempo' : r.daysUntil === 1 ? 'Besok' : `Tersisa ${r.daysUntil} siklus harian`;
    return `
      <div class="recur-item" style="padding:10px 12px;margin-bottom:6px">
        <div class="recur-info">
          <div class="recur-name">${r.name}</div>
          <div class="recur-meta">Tgl ${r.day} | ${label}</div>
        </div>
        <div style="display:flex;align-items:center;gap:12px">
          <div class="recur-amount" style="color:var(--${r.type === 'expense' ? 'red' : 'green'})">${formatRp(r.amount, true)}</div>
          <button class="btn-primary btn-sm" onclick="recordRecurring('${r.id}')"><i class="fa-solid fa-check"></i></button>
        </div>
      </div>
    `;
  }).join('');
...
function renderDashboard() {
  ...
  const headerUser = document.getElementById('welcomeUser');
  if (headerUser) {
    if (s.name) headerUser.textContent = s.name;
    else if (txs.length === 0) headerUser.textContent = 'Selamat Datang';
    else headerUser.textContent = 'Akses Otoritas';
  }
    ...
    wg.innerHTML = Object.keys(w).map(key => {
      ...
      return `
        <div class="wallet-card">
          ...
          <div class="wallet-bal">${formatRp(w[key])}</div>
        </div>
      `;
    }).join('');
  }
  ...
  if (bp) bp.textContent = computeBurnoutPrediction() || 'Data Inkomplet';
  ...
  if (obList) {
    if (!obs.length) {
      obList.innerHTML = '<div style="font-size:13px;color:var(--text-muted);padding:8px 0">Nihil entri kewajiban.</div>';
    } else {
      obList.innerHTML = obs.map(o => `
        <div class="obligation-item ${o.isPaid ? 'paid' : ''}">
          <div class="obligation-info">
            <div class="obligation-check" onclick="toggleObligation('${o.id}')"><i class="fa-solid fa-check"></i></div>
            <div class="obligation-name">${o.name}</div>
          </div>
          <div class="obligation-amount">${formatRp(o.amount)}</div>
        </div>
      `).join('');
    }
  }
  ...
  if (recentEl) {
    if (!sorted.length) {
      recentEl.innerHTML = '<div style="font-size:13px;color:var(--text-muted);padding:8px 0">Rekam entri nihil.</div>';
    } else {
      recentEl.innerHTML = sorted.map(t => renderTxItem(t)).join('');
      recentEl.querySelectorAll('.history-item').forEach(el => el.addEventListener('click', () => openEditModal(el.dataset.id)));
    }
  }
...
function populateWalletSelects() {
  ...
  ['txSourceWallet','txDestWallet','modalTxSourceWallet','modalTxDestWallet','editTxWallet','filterWallet','recurWallet'].forEach(id => {
    ...
    if (!el) return;
    const prev = el.value;
    el.innerHTML = (id === 'filterWallet' ? '<option value="">Seluruh Akun</option>' : '') + optionHtml;
...
function renderDashboardBudgetBars() {
  ...
  if (!entries.length) {
    el.innerHTML = '<div style="font-size:13px;color:var(--text-muted);padding:8px 0">Parameter batas tidak terdeteksi.</div>';
    return;
  }
  const monthTxs = txs.filter(t => getMonthKey(t.date) === currentMonth && t.type === 'expense');
  el.innerHTML = entries.map(([cat, limit]) => {
    ...
    const bgClass = pct >= 90 ? 'var(--red)' : pct >= 70 ? '#f59e0b' : 'var(--green)';
    return `
      <div style="margin-bottom:12px">
        <div style="display:flex;justify-content:space-between;font-size:13px;margin-bottom:6px">
          <span style="font-weight:600;text-transform:capitalize">${cat}</span>
          <span style="color:var(--text-secondary)">${formatRp(spent, true)} / ${formatRp(limit, true)}</span>
        </div>
        <div class="progress-bar-wrap" style="height:6px"><div class="progress-bar" style="width:${pct}%;background-color:${bgClass}"></div></div>
      </div>
    `;
  }).join('');
}
...
function renderDonut(w, total) {
  ...
  const colorPool = ['#3b82f6','#10b981','#8b5cf6','#f59e0b','#ef4444'];
...
function renderTxItem(t) {
  const catIcon = CAT_ICONS[t.category] || 'fa-solid fa-file-invoice';
  ...
  return `
    <div class="history-item" data-id="${t.id}">
      <div class="hi-left">
        <div class="hi-icon ${t.type}"><i class="${catIcon}"></i></div>
        <div class="hi-details">
          <span class="hi-cat">${t.category ? t.category.charAt(0).toUpperCase() + t.category.slice(1) : t.type === 'transfer' ? 'Mutasi' : 'Transaksi'}</span>
          <span class="hi-note">${sub}</span>
        </div>
      </div>
      <div class="hi-right">
        <div class="hi-amount ${t.type}">${amountStr}</div>
        <div class="hi-date">${formatDate(t.date)}</div>
      </div>
    </div>
  `;
}
...
function renderWalletSettings() {
  ...
  if (!list) return;
  const keys = Object.keys(wallets);
  if (!keys.length) { list.innerHTML = '<div style="font-size:13px;color:var(--text-muted)">Data akun nihil.</div>'; return; }
  list.innerHTML = keys.map(k => {
    const m = meta[k] || {};
    const name = m.name || (k === 'cash' ? 'Tunai' : k.toUpperCase());
    const type = m.type || 'cash';
    return `
      <div class="wallet-setting-item" data-wallet="${k}">
        ...
        <select class="form-input wallet-type-select" data-wallet="${k}">
          <option value="bank" ${type==='bank'?'selected':''}>Institusi Finansial</option>
          <option value="cash" ${type==='cash'?'selected':''}>Kas Tunai</option>
          <option value="ewallet" ${type==='ewallet'?'selected':''}>Dompet Digital</option>
        </select>
        <div class="amount-input-wrap"><span class="currency-prefix">Rp</span><input class="form-input amount-input wallet-balance-input" data-wallet="${k}" value="${Number(wallets[k]||0).toLocaleString('id-ID')}"></div>
        <button class="btn-danger-outline" data-wallet="${k}" style="padding:10px" onclick="deleteWallet('${k}')"><i class="fa-solid fa-trash"></i></button>
      </div>
    `;
  }).join('');
...
function deleteWallet(key) {
  showConfirm('Verifikasi Penghapusan', `Cabut otorisasi akun "${key}" dari sistem?`, () => {
    ...
    toast('Otorisasi akun dicabut', 'success');
  });
}
...
function addWalletFromForm() {
  ...
  if (!name) { toast('Deskripsi entitas diperlukan', 'error'); return; }
  const key = slugify(name);
  const wallets = DB.getWallets();
  if (wallets[key] !== undefined) { toast('Duplikasi identitas dilarang', 'warning'); return; }
  ...
  renderWalletSettings(); renderDashboard();
  toast(`Pendaftaran akun "${name}" terkonfirmasi`, 'success');
}
...
function buildCategoryGrid(containerId, type, selectedCat, onSelect) {
  ...
  el.innerHTML = cats.map(c => `
    <div class="cat-btn ${selectedCat === c.id ? 'selected' : ''}" data-cat="${c.id}">
...
function setTxType(type, context) {
  ...
  document.getElementById(isModal ? 'modalSourceLabel' : 'sourceWalletLabel').textContent = type === 'transfer' ? 'Pusat Transmisi' : 'Pusat Akun';
...
function submitTransaction(type, dateId, amountId, srcWalletId, destWalletId, noteId, catGetter, tagsId) {
  ...
  if (!amount || amount <= 0) { toast('Verifikasi nilai tidak valid', 'error'); return false; }
  if (!date) { toast('Tentukan periode pelaksanaan', 'error'); return false; }
  if (type !== 'transfer' && !category) { toast('Identifikasi jenis diperlukan', 'warning'); return false; }
  const wallets = DB.getWallets();
  const txs = DB.getTransactions();
  if (type === 'expense') {
    if ((wallets[sourceWallet] || 0) < amount) { toast('Capaian limit ekuitas ditolak', 'warning'); return false; }
    ...
    if (sourceWallet === destWallet) { toast('Gagal: Entitas transmisi identik', 'error'); return false; }
    if ((wallets[sourceWallet] || 0) < amount) { toast('Capaian limit ekuitas ditolak', 'warning'); return false; }
    wallets[sourceWallet] -= amount;
...
function checkBudgetAlert(category, amount) {
  ...
  if (pct >= 100) setTimeout(() => toast(`Status Limit: Tercapai (${category})`, 'error'), 600);
  else if (pct >= 80) setTimeout(() => toast(`Status Limit: Tersisa ${Math.round(100-pct)}% (${category})`, 'warning'), 600);
}
...
function renderHistory() {
  ...
  if (hs) hs.innerHTML = `
    <div class="hs-stat"><div class="hs-stat-label">Inflow</div><div class="hs-stat-val in">${formatRp(totalIn, true)}</div></div>
    <div class="hs-stat"><div class="hs-stat-label">Outflow</div><div class="hs-stat-val out">${formatRp(totalOut, true)}</div></div>
    <div class="hs-stat"><div class="hs-stat-label">Surplus/Defisit</div><div class="hs-stat-val" style="color:${net>=0?'var(--green)':'var(--red)'}">${net >= 0 ? '+' : ''}${formatRp(net, true)}</div></div>
  `;
  const listEl = document.getElementById('historyList');
  if (!listEl) return;
  if (!filtered.length) { listEl.innerHTML = '<div style="text-align:center;padding:24px;color:var(--text-muted);font-size:14px;">Kueri data menghasilkan nol himpunan.</div>'; return; }
  const grouped = {};
    ...
    return `
      <div>
        <div class="history-date-header" style="display:flex;justify-content:space-between">
          ...
          <span style="color:${dayTotal>=0?'var(--green)':'var(--red)'}">${dayTotal>=0?'+':''}${formatRp(dayTotal,true)}</span>
        </div>
        ${items.map(t => renderTxItem(t)).join('')}
      </div>
    `;
  }).join('');
  listEl.querySelectorAll('.history-item').forEach(el => el.addEventListener('click', () => openEditModal(el.dataset.id)));
}
...
function populateCatFilter() {
  ...
  if (!el) return;
  el.innerHTML = '<option value="">Semua Filter Kategori</option>' +
...
function renderCalendar() {
  ...
  for (let d = 1; d <= days; d++) {
    ...
    const dt = txByDay[ds] || [];
    let dots = dt.some(t => t.type === 'expense') ? '<div class="cal-dot" style="background:var(--red)"></div>' : '';
    dots += dt.some(t => t.type === 'income') ? '<div class="cal-dot" style="background:var(--green)"></div>' : '';
    dots += dt.some(t => t.type === 'transfer') ? '<div class="cal-dot" style="background:var(--primary)"></div>' : '';
    cells += `<div class="cal-cell ${ds === today ? 'today' : ''} ${ds === selectedCalDay ? 'selected' : ''}" data-date="${ds}">
...
function showCalDayDetail(dateStr, dayTxs) {
  ...
  if (!el) return;
  if (!dayTxs.length) { el.innerHTML = '<div style="text-align:center;padding:16px;font-size:13px;color:var(--text-muted)">Catatan nihil pada siklus ini.</div>'; return; }
  el.innerHTML = dayTxs.map(t => renderTxItem(t)).join('');
  el.querySelectorAll('.history-item').forEach(item => item.addEventListener('click', () => openEditModal(item.dataset.id)));
}
...
function renderBudgetPage() {
  ...
  if (el) {
    if (!entries.length) {
      el.innerHTML = '<div style="text-align:center;padding:24px;font-size:13px;color:var(--text-muted)">Indikator limit belum terdaftar.</div>';
    } else {
      el.innerHTML = entries.map(([cat, limit]) => {
        const spent = monthTxs.filter(t => t.category === cat).reduce((a, t) => a + t.amount, 0);
        const pct = limit > 0 ? Math.min(100, (spent / limit) * 100) : 0;
        ...
        return `
          <div class="budget-item">
            <div class="budget-header">
              <span class="budget-cat" style="text-transform:capitalize">${cat}</span>
              <div class="budget-amounts"><span class="budget-used">${formatRp(spent, true)}</span> <span class="budget-limit">/ ${formatRp(limit, true)}</span></div>
            </div>
            <div class="budget-bar-wrap"><div class="budget-bar" style="width:${pct}%;background-color:${bgClass}"></div></div>
            <div style="text-align:right;margin-top:8px"><button class="btn-danger-outline btn-sm" onclick="deleteBudget('${cat}')"><i class="fa-solid fa-trash"></i></button></div>
          </div>
        `;
      }).join('');
    }
...
function deleteBudget(cat) {
  ...
  renderBudgetPage(); toast(`Batas alokasi ${cat} dibatalkan`, 'success');
}
...
function renderBudgetChart(budgets, monthTxs) {
  ...
  budgetChart = new Chart(ctx, {
    ...
    data: { labels, datasets: [
      { label: 'Utilisasi', data: spentData, backgroundColor: '#ef4444', borderRadius: 4 },
      { label: 'Batas Indeks', data: limitData, backgroundColor: 'rgba(59,130,246,0.3)', borderRadius: 4 }
    ]},
    options: { responsive: true, maintainAspectRatio: false,
      plugins: { legend: { labels: { color: '#64748b', font: { size: 11 } } } },
      scales: {
        x: { ticks: { color: '#64748b', font: { size: 11 } }, grid: { color: 'rgba(255,255,255,0.05)' } },
        y: { ticks: { color: '#64748b', font: { size: 11 }, callback: v => formatRp(v, true) }, grid: { color: 'rgba(255,255,255,0.05)' } }
      }
    }
  });
}
...
function renderGoalsPage() {
  ...
  if (!el) return;
  if (!goals.length) { el.innerHTML = '<div style="text-align:center;padding:24px;font-size:13px;color:var(--text-muted)">Rekaman target nihil.</div>'; return; }
  el.innerHTML = goals.map(g => {
    const pct = g.target > 0 ? Math.min(100, Math.round((g.current / g.target) * 100)) : 0;
    return `
      <div class="goal-card">
        <div class="goal-header">
          <span class="goal-name"><i class="fa-solid fa-flag-checkered" style="margin-right:8px;color:var(--text-secondary)"></i>${g.name}</span>
          <span class="goal-pct">${pct}%</span>
        </div>
        <div class="goal-meta">${g.deadline ? `Realisasi: ${formatDate(g.deadline)}` : 'Siklus terbuka'}</div>
        <div class="progress-bar-wrap"><div class="progress-bar" style="width:${pct}%;background-color:var(--green)"></div></div>
        <div style="display:flex;justify-content:space-between;font-size:13px;margin:8px 0 16px;">
          <span>Ekuitas Aktif: <strong>${formatRp(g.current, true)}</strong></span>
          <span style="color:var(--text-secondary)">Indeks: ${formatRp(g.target, true)}</span>
        </div>
        <div style="display:flex;gap:12px;border-top:1px solid var(--border);padding-top:16px;">
          <div class="amount-input-wrap" style="flex:1"><span class="currency-prefix">Rp</span><input class="form-input amount-input goal-topup-input" id="topup_${g.id}" placeholder="Nilai injeksi"></div>
          <button class="btn-primary" onclick="topupGoal('${g.id}')"><i class="fa-solid fa-plus"></i></button>
          <button class="btn-danger-outline" onclick="deleteGoal('${g.id}')"><i class="fa-solid fa-trash"></i></button>
        </div>
      </div>
    `;
  }).join('');
...
function deleteGoal(id) {
  showConfirm('Penghapusan Indeks', 'Validasi instruksi penghapusan objektif finansial?', () => {
    DB.saveGoals(DB.getGoals().filter(g => g.id !== id));
    renderGoalsPage(); renderDashGoals(); toast('Indeks sasaran dieliminasi', 'success');
  });
}
...
function topupGoal(id) {
  ...
  if (!amount || amount <= 0) { toast('Injeksi nilai invalid', 'error'); return; }
  const goals = DB.getGoals();
  ...
  toast(`Nilai ${formatRp(amount, true)} diakuisisi ke ${g.name}`, 'success');
  if (g.current >= g.target) setTimeout(() => toast(`Status limit terlampaui: ${g.name}`, 'success'), 500);
}
...
function renderRecurringPage() {
  ...
  if (!el) return;
  if (!recurrings.length) { el.innerHTML = '<div style="text-align:center;padding:24px;font-size:13px;color:var(--text-muted)">Rekaman instruksi nihil.</div>'; return; }
  el.innerHTML = recurrings.map(r => `
    <div class="recur-item">
      <div class="recur-info">
        <span class="recur-name">${r.name}</span>
        <span class="recur-meta">Siklus: ${r.day} | Saluran: ${getWalletName(r.wallet)}</span>
      </div>
      <div style="display:flex;align-items:center;gap:16px;">
        <span class="recur-amount" style="color:var(--${r.type === 'expense' ? 'red' : 'green'})">${r.type === 'expense' ? '-' : '+'}${formatRp(r.amount, true)}</span>
        <div style="display:flex;gap:8px;">
          <button class="btn-primary btn-sm" onclick="recordRecurring('${r.id}')"><i class="fa-solid fa-check"></i></button>
          <button class="btn-danger-outline btn-sm" style="padding:8px" onclick="deleteRecurring('${r.id}')"><i class="fa-solid fa-trash"></i></button>
        </div>
      </div>
    </div>
  `).join('');
...
function deleteRecurring(id) {
  showConfirm('Penghapusan Entri', 'Validasi terminasi perintah periodik?', () => {
    DB.saveRecurring(DB.getRecurring().filter(r => r.id !== id));
    renderRecurringPage(); toast('Perintah terminasi dilaksanakan', 'success');
  });
}
...
function renderNetWorthPage() {
  ...
  if (ns) ns.textContent = netWorth >= 0 ? 'Indeks Terkonfirmasi Surplus' : 'Indeks Terkonfirmasi Defisit';
  ...
  if (assetList) {
    let html = `<div class="nw-asset-item"><span>Seluruh Integrasi Akun</span><span style="font-weight:700;color:var(--green)">${formatRp(walletTotal, true)}</span></div>`;
    assets.forEach(a => {
      html += `<div class="nw-asset-item"><span>${a.name}</span><div style="display:flex;align-items:center;gap:12px"><span style="font-weight:700;color:var(--green)">${formatRp(a.value, true)}</span><i class="fa-solid fa-trash" style="cursor:pointer;color:var(--red)" onclick="deleteNWItem('asset','${a.id}')"></i></div></div>`;
    });
  ...
  if (debtList) {
    if (!debts.length) { debtList.innerHTML = '<div style="font-size:13px;color:var(--text-muted);margin-top:8px">Data liabilitas eksternal kosong.</div>'; }
    else {
      debtList.innerHTML = debts.map(d => `
        <div class="nw-asset-item"><span>${d.name}</span><div style="display:flex;align-items:center;gap:12px"><span style="font-weight:700;color:var(--red)">${formatRp(d.value, true)}</span><i class="fa-solid fa-trash" style="cursor:pointer;color:var(--red)" onclick="deleteNWItem('debt','${d.id}')"></i></div></div>
      `).join('');
    }
  }
  ...
  networthChartInst = new Chart(ctx, {
    ...
    options: { responsive: true, maintainAspectRatio: false,
      ...
      scales: {
        ...
        y: { ticks: { color: '#64748b', font: { size: 10 }, callback: v => formatRp(v, true) }, grid: { color: 'rgba(255,255,255,0.05)' } }
      }
    }
  });
}
...
function renderSplitPersonList() {
  ...
  el.innerHTML = splitPersons.map((p, i) => `
    <div class="split-person-row">
      ...
      <i class="fa-solid fa-xmark" style="cursor:pointer;color:var(--text-secondary)" onclick="removeSplitPerson(${i})"></i>
    </div>
  `).join('') || '<div style="font-size:13px;color:var(--text-muted);padding:8px">Identifikasi nihil.</div>';
}
...
function calculateSplit() {
  ...
  if (!total || !splitPersons.length) { toast('Terdapat variabel absolut yang hilang', 'warning'); return; }
  ...
  res.innerHTML = `
    <div class="card card-flat" style="border-color:var(--primary)">
      <div style="font-weight:700;font-size:16px;margin-bottom:8px">${document.getElementById('splitTitle')?.value || 'Atribut Distribusi Beban'}</div>
      <div style="font-size:13px;color:var(--text-secondary);margin-bottom:16px">Toleransi Nominal: ${formatRp(total)} | Diskon: ${formatRp(discount)} | Pajak: ${taxPct}% <br><strong>Agregat Akhir: ${formatRp(afterTax)}</strong></div>
      ${splitPersons.map(p => `
        <div class="split-res-item">
          <span style="font-weight:600">${p}</span>
          <span style="font-family:var(--font-display);font-weight:700;color:var(--primary)">${formatRp(Math.ceil(perPerson))}</span>
        </div>
      `).join('')}
    </div>
  `;
...
function hitungCicilan() {
  ...
  if (!pokok || !tenor) { toast('Lengkapi kueri kalkulasi', 'error'); return; }
  const pinjaman = pokok - dp;
  if (pinjaman <= 0) { toast('Konflik: Reduksi melebihi beban agregat', 'error'); return; }
  ...
  if (tbody) tbody.innerHTML = rows.map(r => `<tr><td style="text-align:center">${r.bulan}</td><td>${formatRp(r.angsuran, true)}</td><td>${formatRp(r.pokok, true)}</td><td>${formatRp(r.bunga, true)}</td><td>${formatRp(r.sisa, true)}</td></tr>`).join('');
...
function renderCicilanChart(rows) {
  ...
  cicilanChart = new Chart(ctx, {
    type: 'line',
    data: { labels: rows.map(r => `P.${r.bulan}`), datasets: [{ label: 'Residu Liabilitas', data: rows.map(r => r.sisa), borderColor: '#8b5cf6', backgroundColor: 'rgba(139,92,246,0.1)', fill: true, tension: 0.3, pointRadius: 0 }] },
    options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { x: { ticks: { color: '#64748b', font: { size: 10 }, maxTicksLimit: 12 }, grid: { color: 'rgba(255,255,255,0.05)' } }, y: { ticks: { color: '#64748b', font: { size: 10 }, callback: v => formatRp(v, true) }, grid: { color: 'rgba(255,255,255,0.05)' } } } }
  });
}
...
function renderQuickTemplateBar() {
  ...
  el.innerHTML = templates.map(t => `<div class="template-btn" data-id="${t.id}"><i class="${CAT_ICONS[t.category] || 'fa-solid fa-layer-group'}"></i>${t.name}</div>`).join('');
  el.querySelectorAll('.template-btn').forEach(chip => chip.addEventListener('click', () => applyTemplate(chip.dataset.id)));
}
...
function applyTemplate(id) {
  ...
  setTimeout(() => {
    document.querySelectorAll('#categoryGrid .cat-btn').forEach(c => c.classList.toggle('selected', c.dataset.cat === tpl.category));
  }, 50);
  toast(`Registri otomatis memuat profil: ${tpl.name}`, 'success');
}
...
function renderTemplateModal() {
  ...
  if (!el) return;
  if (!templates.length) { el.innerHTML = '<div style="text-align:center;padding:24px;font-size:13px;color:var(--text-muted)">Rekaman instan nihil.</div>'; return; }
  el.innerHTML = templates.map(t => `
    <div class="history-item" data-id="${t.id}" style="margin-bottom:8px">
      <div class="hi-left">
        <div class="hi-icon ${t.type}"><i class="${CAT_ICONS[t.category] || 'fa-solid fa-layer-group'}"></i></div>
        <div class="hi-details">
          <span class="hi-cat">${t.name}</span>
          <span class="hi-note">${t.category} | ${getWalletName(t.wallet)} | ${t.type}</span>
        </div>
      </div>
      <div style="display:flex;align-items:center;gap:12px">
        <span class="hi-amount" style="color:var(--red)">${formatRp(t.amount, true)}</span>
        <button class="btn-danger-outline btn-sm" onclick="deleteTemplate('${t.id}');event.stopPropagation()"><i class="fa-solid fa-trash"></i></button>
      </div>
    </div>
  `).join('');
...
function deleteTemplate(id) {
  ...
  toast('Repositori otomatis didelete', 'success');
}
...
async function callAI(prompt, data) {
  try {
    ...
    if (!res.ok) throw new Error('Kode interupsi transmisi: ' + res.statusText);
    const json = await res.json();
    return json.result || 'Resolusi pemrosesan AI merespons hasil kosong';
  } catch (e) { toast('Kesalahan terminal AI: ' + e.message, 'error'); throw e; }
}
...
const AI_PROMPTS = {
  summary: 'Hasilkan ringkasan absolut. Ekstraksi esensi pemrosesan kuantitatif tanpa basa-basi.',
  predict: 'Eksekusi estimasi cadangan kas akhir dari dataset. Proyeksikan resiko kepailitan operasional bulanan.',
  tips: 'Generasikan algoritma solusi korektif. Susun tiga direktif terstruktur penghentian anomali outflow.',
  anomaly: 'Pindai blok log data. Saring deviasi nilai absolut dan presentasikan tanggal anomali spesifik.',
  cashflow: 'Ekstraksi arus agregat triwulanan. Validasi tren stabilitas nilai ekuivalen kas murni.',
  goal: 'Analisis logika penetapan variabel target. Beri keputusan valid/invalid dengan referensi numerik jelas.'
};
...
async function runAIInsight(promptKey, customPrompt) {
  ...
  try {
    ...
    if (content) { content.style.display = 'block'; content.innerHTML = result; }
  } catch (e) {
    if (loading) loading.style.display = 'none';
    if (content) { content.style.display = 'block'; content.innerHTML = `<div style="color:var(--red);font-weight:600">Pemutusan koneksi terdeteksi: ${e.message}</div>`; }
  }
}
...
function initSettingsNav() {
  ...
  const subContainer = document.getElementById('settingsSubMenuContainer');
  const backBtn = document.getElementById('settingsBackBtn');
  const subTitle = document.getElementById('settingsSubTitle');
  const subPages = document.querySelectorAll('.settings-sub-page');
  ...
  items.forEach(item => {
    item.addEventListener('click', () => {
      ...
      if (targetId === 'settingsPersonal') {
        document.getElementById('settingName').value = DB.getSettings().name || '';
      }
      if (targetId === 'settingsParams') {
        const s = DB.getSettings();
      ...
      if (targetId === 'settingsObligations') {
        renderObligationsSettings();
      }
      if (targetId === 'settingsWallets') {
...
function renderObligationsSettings() {
  const obs = DB.getObligations();
  const obsEl = document.getElementById('obligationsSettings');
  if (!obsEl) return;
  if (!obs.length) { obsEl.innerHTML = '<div style="font-size:13px;color:var(--text-muted)">Rekaman beban nihil.</div>'; }
  else {
    obsEl.innerHTML = obs.map(o => `
      <div style="display:flex;justify-content:space-between;align-items:center;padding:12px;border-bottom:1px solid var(--border)">
        <span style="font-size:14px;font-weight:500">${o.name}</span>
        <div style="display:flex;align-items:center;gap:12px">
          <span style="font-weight:700">${formatRp(o.amount)}</span>
          <i class="fa-solid fa-trash" style="cursor:pointer;color:var(--red)" onclick="deleteObligation('${o.id}')"></i>
        </div>
      </div>
    `).join('');
  }
...
function deleteObligation(id) {
  showConfirm('Otorisasi Penghapusan', 'Konfirmasi pemusnahan basis data kewajiban?', () => {
    DB.saveObligations(DB.getObligations().filter(o => o.id !== id));
    renderObligationsSettings(); renderDashboard();
    toast('Data kewajiban diputus dari sistem', 'success');
  });
}
...
async function generatePDFReport() {
  ...
  container.innerHTML = `
    <div style="font-family:Arial,sans-serif;color:#111;max-width:760px;margin:0 auto">
      <div style="background:#0f172a;color:white;padding:28px 32px;border-radius:0">
        <div style="font-size:24px;font-weight:800;letter-spacing:-0.5px">Sistem Analitik MyWallet</div>
        <div style="font-size:14px;opacity:0.8;margin-top:4px">Laporan Eksekusi Dokumen Kas</div>
        <div style="font-size:16px;font-weight:700;margin-top:8px">Periode: ${formatMonthLabel(cm)} | Otoritas: ${s.name || 'Sistem Dasar'}</div>
      </div>
      <div style="background:#f1f5f9;padding:24px 32px;display:grid;grid-template-columns:1fr 1fr 1fr;gap:16px">
        <div style="background:white;padding:16px;border:1px solid #cbd5e1">
          <div style="font-size:11px;color:#475569;font-weight:700;text-transform:uppercase;margin-bottom:4px">Arus Inflow</div>
          <div style="font-size:20px;font-weight:800;color:#059669">${formatRp(income)}</div>
        </div>
        <div style="background:white;padding:16px;border:1px solid #cbd5e1">
          <div style="font-size:11px;color:#475569;font-weight:700;text-transform:uppercase;margin-bottom:4px">Arus Outflow</div>
          <div style="font-size:20px;font-weight:800;color:#dc2626">${formatRp(expense)}</div>
        </div>
        <div style="background:white;padding:16px;border:1px solid #cbd5e1">
          <div style="font-size:11px;color:#475569;font-weight:700;text-transform:uppercase;margin-bottom:4px">Deviasi Net</div>
          <div style="font-size:20px;font-weight:800;color:${net >= 0 ? '#059669' : '#dc2626'}">${net >= 0 ? '+' : ''}${formatRp(net)}</div>
        </div>
      </div>
      <div style="background:white;padding:24px 32px;border-top:1px solid #cbd5e1">
        <div style="font-size:14px;font-weight:800;margin-bottom:16px;color:#0f172a">Skala Pengeluaran Induk</div>
        ${topCats.map(([cat, amt]) => `
      ...
      <div style="background:#f1f5f9;padding:20px 32px;display:flex;justify-content:space-between;align-items:center;border-top:1px solid #cbd5e1">
        <div style="font-size:13px;color:#475569">Rasio Likuiditas Sistem: <strong style="color:#2563eb;font-size:18px">${healthScore}/100</strong></div>
        <div style="font-size:13px;color:#475569">Defisit Operasional Toleransi: <strong>${formatRp(duitFree)}</strong></div>
      </div>
      <div style="background:#0f172a;color:rgba(255,255,255,0.6);padding:12px 32px;text-align:right;font-size:11px">
        Terminal Log Generation · ${new Date().toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })}
      </div>
    </div>
  `;

  const printWin = window.open('', '_blank');
  if (!printWin) { toast('Akses transmisi popup terblokir sistem', 'warning'); return; }
  printWin.document.write(`<!DOCTYPE html><html><head><title>Laporan Siklus ${formatMonthLabel(cm)}</title><style>body{margin:0;padding:20px;background:#fff}*{box-sizing:border-box}@media print{body{padding:0}}</style></head><body>${container.innerHTML}</body></html>`);
  printWin.document.close();
  setTimeout(() => { printWin.print(); }, 300);
  toast('Dokumen persiapan PDF selesai', 'success');
}

async function enableNotifications() {
  const statusEl = document.getElementById('notifStatusText');
  if (!('Notification' in window)) {
    if (statusEl) statusEl.textContent = 'Penolakan kernel terhadap push notifikasi';
    return;
  }
  const perm = await Notification.requestPermission();
  if (perm === 'granted') {
    if (statusEl) statusEl.textContent = 'Protokol Interupsi Sinkronisasi: Terotorisasi';
    toast('Jalur notifikasi terminal dipulihkan', 'success');
    scheduleNotifications();
  } else {
    if (statusEl) statusEl.textContent = 'Ditolak klien. Instruksi manual diwajibkan melalui konfigurasi browser.';
  }
}
...
function scheduleNotifications() {
  ...
  if (!todayTxs.length) {
    setTimeout(() => {
      if (Notification.permission === 'granted') new Notification('MyWallet Terminal', { body: 'Inisialisasi nol: Data input nihil untuk siklus saat ini.' });
    }, 2000);
  }
  const unpaid = obs.filter(o => !o.isPaid);
  if (unpaid.length) {
    setTimeout(() => {
      if (Notification.permission === 'granted') new Notification('MyWallet Terminal', { body: `Anomali Liabilitas: Terdapat ${unpaid.length} entri wajib yang membekukan dana operasi.` });
    }, 3000);
  }
  const monthTxs = txs.filter(t => getMonthKey(t.date) === cm && t.type === 'expense');
  Object.entries(budgets).forEach(([cat, limit]) => {
    ...
    if (pct >= 80) {
      setTimeout(() => {
        if (Notification.permission === 'granted') new Notification('MyWallet Terminal', { body: `Kritikal Toleransi: Kategori ${cat} terekskusi pada level ${Math.round(pct)}%.` });
      }, 4000);
    }
  });
}
...
function applyIntent(intent) {
  ...
  if (intent.type === 'expense') {
    ...
    if (!src) return { ok: false, msg: 'Target akun nihil' };
    if ((wallets[src] || 0) < intent.amount) return { ok: false, msg: 'Defisit alokasi terdeteksi' };
    wallets[src] -= intent.amount;
    txs.push({ id: DB.generateId(), type: 'expense', amount: intent.amount, sourceWallet: src, destWallet: null, category: intent.category || 'lainnya', note: 'Via Modul Teks', date, tags: [] });
    DB.saveWallets(wallets); DB.saveTransactions(txs);
    return { ok: true, label: getWalletName(src) };
  }
  if (intent.type === 'income') {
    const dest = intent.destWallet || intent.sourceWallet || Object.keys(wallets)[0];
    if (!dest) return { ok: false, msg: 'Target akun nihil' };
    wallets[dest] = (wallets[dest] || 0) + intent.amount;
    txs.push({ id: DB.generateId(), type: 'income', amount: intent.amount, sourceWallet: dest, destWallet: null, category: intent.category || 'gaji', note: 'Via Modul Teks', date, tags: [] });
    DB.saveWallets(wallets); DB.saveTransactions(txs);
    return { ok: true, label: getWalletName(dest) };
  }
  if (intent.type === 'transfer') {
    const src = intent.sourceWallet, dst = intent.destWallet;
    if (!src || !dst) return { ok: false, msg: 'Fragmentasi data rute tidak valid' };
    if ((wallets[src] || 0) < intent.amount) return { ok: false, msg: 'Defisit alokasi transmisi' };
    wallets[src] -= intent.amount;
    wallets[dst] = (wallets[dst] || 0) + intent.amount;
    txs.push({ id: DB.generateId(), type: 'transfer', amount: intent.amount, sourceWallet: src, destWallet: dst, category: 'transfer', note: 'Via Modul Teks', date, tags: [] });
    DB.saveWallets(wallets); DB.saveTransactions(txs);
    return { ok: true, labelSrc: getWalletName(src), labelDst: getWalletName(dst) };
  }
  return { ok: false, msg: 'Parameter jenis tertolak' };
}
...
function renderPendingChatUI() {
  ...
  container.style.cssText = 'padding:12px;border:1px solid var(--border);background:var(--bg-base);border-radius:4px;margin-top:8px;font-size:13px';
  let inner = '<div style="font-weight:700;margin-bottom:12px;color:var(--text-secondary)">Aksi Tunda (Otorisasi Diperlukan):</div><div style="display:flex;flex-direction:column;gap:12px">';
  pendingChatTxs.forEach(p => {
    const it = p.intent;
    let desc = '';
    if (it.type === 'expense') desc = `Pengurangan ${formatRp(it.amount)} dari ${getWalletName(it.sourceWallet)}`;
    else if (it.type === 'income') desc = `Penambahan ${formatRp(it.amount)} menuju ${getWalletName(it.destWallet || it.sourceWallet)}`;
    else if (it.type === 'transfer') desc = `Transmisi ${formatRp(it.amount)} dari ${getWalletName(it.sourceWallet)} menuju ${getWalletName(it.destWallet)}`;
    inner += `<div style="display:flex;justify-content:space-between;align-items:center;"><span>${desc}</span><span style="display:flex;gap:4px"><button class="btn-primary btn-sm" onclick="confirmPendingChat('${p.id}')">Validasi</button><button class="btn-danger-outline btn-sm" onclick="ignorePendingChat('${p.id}')">Tolak</button></span></div>`;
  });
  inner += '</div>';
...
function confirmPendingChat(pid) {
  ...
  if (!res.ok) { appendChatLog(`Kegagalan Eksekusi: ${res.msg}`, 'bot'); }
  else if (item.intent.type === 'transfer') appendChatLog(`Klarifikasi Mutasi: ${formatRp(item.intent.amount)} ${res.labelSrc} -> ${res.labelDst}`, 'bot');
  else appendChatLog(`Pencatatan sukses: ${item.intent.type === 'income' ? '+' : '-'}${formatRp(item.intent.amount)}`, 'bot');
...
function ignorePendingChat(pid) {
  const idx = pendingChatTxs.findIndex(p => p.id === pid);
  if (idx === -1) return;
  appendChatLog(`Aksi diinterupsi spesifik`, 'bot');
  pendingChatTxs.splice(idx, 1);
  renderPendingChatUI();
}
...
function handleChatInput() {
  ...
  if (!intents.length) {
    appendChatLog('Terminal menolak kueri: Identifikasi angka nominal nihil.', 'bot');
    input.value = '';
    return;
  }
  const wallets = DB.getWallets();
  ...
  if (toApply.length) {
    toApply.forEach(a => {
      ...
      if (!res.ok) appendChatLog(`Gagal komputasi: ${res.msg}`, 'bot');
      else if (a.type === 'transfer') appendChatLog(`Mutasi diproses: ${formatRp(a.amount)}`, 'bot');
      else appendChatLog(`Status rekam: ${a.type === 'income' ? '+' : '-'}${formatRp(a.amount)}`, 'bot');
    });
    renderDashboard();
  }
  if (toConfirm.length) {
    toConfirm.forEach(it => pendingChatTxs.push({ id: DB.generateId(), intent: { ...it, date: dateToUse } }));
    appendChatLog('Bypass sistem terdeteksi. Silakan klarifikasi izin:', 'bot');
    renderPendingChatUI();
  }
  input.value = '';
...
function renderOnboardingModal() {
  ...
  if (!container) return;
  container.innerHTML = `
    <div style="font-size:14px;color:var(--text-secondary);margin-bottom:20px;line-height:1.6">Sistem basis membutuhkan identifikasi dasar dan inisialisasi awal komoditas (dompet) operasional sebelum memasuki dasbor utama.</div>
    <div class="form-group">
      <label class="form-label">Kredensial Subjek</label>
      <input id="onboardName" class="form-input" placeholder="Masukkan nama pengguna..." value="${settings.name || ''}">
    </div>
    <div class="form-group" style="margin-top:24px;border-top:1px solid var(--border);padding-top:20px">
      <label class="form-label">Pusat Akun Primer</label>
      <div style="display:flex;gap:12px;align-items:flex-start">
        <input id="onboardNewWalletName" class="form-input" placeholder="Label Dompet Dasar" style="flex:1">
        <div class="amount-input-wrap" style="flex:1"><span class="currency-prefix">Rp</span><input id="onboardNewWalletBalance" class="form-input amount-input" placeholder="Nilai Awal" value="0"></div>
      </div>
    </div>
    <div style="display:flex;justify-content:flex-end;gap:12px;margin-top:32px">
      <button class="btn-secondary" id="onboardingSkipBtn">Bypass Validasi</button>
      <button class="btn-primary" id="onboardingSaveBtn">Terapkan Konfigurasi</button>
    </div>
  `;
  container.querySelectorAll('.amount-input').forEach(inp => inp.addEventListener('input', () => formatInputRp(inp)));
  container.querySelector('#onboardingSaveBtn')?.addEventListener('click', () => {
    const name = (document.getElementById('onboardName')?.value || '').trim();
    const wn = (document.getElementById('onboardNewWalletName')?.value || '').trim();
    const wv = parseRpInput(document.getElementById('onboardNewWalletBalance')?.value || '0');
    if (wn) {
      const key = slugify(wn);
      const w = DB.getWallets(), m = DB.getWalletMeta();
      w[key] = wv;
      m[key] = { name: wn, type: 'cash', synonyms: [] };
      DB.saveWallets(w); DB.saveWalletMeta(m);
    }
    DB.saveSettings({ ...DB.getSettings(), name: name || '', _onboarded: 1 });
    hideModal('onboardingModal');
    renderWalletSettings(); renderDashboard();
    toast('Konfigurasi blok dasar diinisialisasi', 'success');
  });
...
function showOnboardingIfNeeded() {
  ...
  if (!s._onboarded && walletKeys.length === 0) { renderOnboardingModal(); showModal('onboardingModal'); }
}
...
function exportData() {
  ...
  toast('Blok ekspor sukses tergenerate', 'success');
}
...
function importData(file) {
  ...
  reader.onload = e => {
    try {
      ...
      toast('Overriding blok data berhasil', 'success');
      navigateTo('dashboard');
    } catch { toast('Pola JSON destruktif tertolak', 'error'); }
  };
...
function initPWA() {
  ...
  const promptInstall = async () => {
    if (!pwaPrompt) { toast('Tindakan ini memerlukan instruksi native browser', 'info'); return; }
    pwaPrompt.prompt();
    const { outcome } = await pwaPrompt.userChoice;
    if (outcome === 'accepted') { document.getElementById('pwa-install-bar').style.display = 'none'; toast('Aplikasi sistem diadopsi native', 'success'); }
    pwaPrompt = null;
  };
  window.addEventListener('beforeinstallprompt', e => {
    ...
    if (!dismissed && bar && !isStandalone()) bar.style.display = 'flex';
  });
...
function navigateTo(page) {
  ...
  const titles = { dashboard: 'Dasbor Utama', transactions: 'Proses Entri Transaksional', history: 'Buku Besar Transaksional', settings: 'Konfigurasi Inti Sistem', calendar: 'Penjadwalan Log', budget: 'Pembatasan Alokasi', cicilan: 'Kalkulator Amortisasi', ai: 'Operasi Pemrosesan AI', goals: 'Proyeksi Target Finansial', recurring: 'Manajemen Beban Berulang', networth: 'Kalkulasi Kekayaan Bersih', splitbill: 'Pembagian Proporsi Tagihan' };
...
function initApp() {
  ...
  document.getElementById('chatDateBtn')?.addEventListener('click', () => {
    ...
    appendChatLog(`Parameter set timer: ${new Date(chatSelectedDate + 'T00:00:00').toLocaleDateString('id-ID')}`, 'bot');
  });
  ...
  document.getElementById('modalTransactionForm')?.addEventListener('submit', e => {
    ...
    if (ok) {
      ...
      toast('Rekam entitas solid sukses', 'success');
      const { duitFree } = computeDuitFree();
      if (duitFree < 0) setTimeout(() => toast('Toleransi indeks likuiditas kolaps', 'warning'), 500);
      renderDashboard();
    }
  });
  ...
  document.getElementById('saveAsTemplate')?.addEventListener('change', function() {
    ...
    if (tn) tn.style.display = this.checked ? 'block' : 'none';
  });
  ...
  document.getElementById('transactionForm')?.addEventListener('submit', e => {
    ...
    if (ok) {
      ...
      if (saveAsTpl && currentTxType !== 'transfer') {
        const tplName = document.getElementById('templateName')?.value.trim() || currentTxCategory || 'Entri Instan';
        const templates = DB.getTemplates();
        ...
        toast(`Identifikasi repositori termuat`, 'success');
      ...
      toast('Validasi pencatatan log lengkap', 'success');
      const { duitFree } = computeDuitFree();
      if (duitFree < 0) setTimeout(() => toast('Toleransi indeks likuiditas kolaps', 'warning'), 500);
      renderDashboard();
    }
  ...
  document.getElementById('editTransactionForm')?.addEventListener('submit', e => {
    ...
    if (!newAmount || newAmount <= 0) { toast('Nominal parameter tertolak', 'error'); return; }
    const txs = DB.getTransactions();
    ...
    if (old.type === 'expense') {
      ...
      if ((wallets[newWallet] || 0) < newAmount) { toast('Resolusi indeks terlampaui', 'warning'); wallets[old.sourceWallet] -= old.amount; return; }
    ...
    DB.saveWallets(wallets); DB.saveTransactions(txs);
    hideModal('editModal');
    toast('Data spesifik termodifikasi', 'success');
    renderDashboard();
  ...
  document.getElementById('deleteTransaction')?.addEventListener('click', () => {
    const id = document.getElementById('editTxId')?.value;
    showConfirm('Inisiasi Eksekusi Penghapusan', 'Resolusi tindakan menyebabkan pengembalian status saldo dan penghapusan log riwayat permanen. Lanjutkan?', () => {
      const txs = DB.getTransactions();
      const idx = txs.findIndex(t => t.id === id);
      if (idx === -1) return;
      const old = txs[idx];
      ...
      DB.saveWallets(wallets); DB.saveTransactions(txs);
      hideModal('editModal');
      toast('Elemen log dihancurkan sepenuhnya', 'success');
      renderDashboard();
      if (document.getElementById('page-history')?.classList.contains('active')) renderHistory();
      if (document.getElementById('page-calendar')?.classList.contains('active')) renderCalendar();
    });
  ...
  document.getElementById('saveBudget')?.addEventListener('click', () => {
    ...
    if (!amount || amount <= 0) { toast('Tentukan limit resolusi absolut', 'error'); return; }
    const budgets = DB.getBudgets();
    budgets[cat] = amount;
    ...
    if (ba) ba.value = '';
    renderBudgetPage();
    toast(`Injeksi restriksi kategori dieksekusi`, 'success');
  });
  ...
  document.getElementById('saveGoal')?.addEventListener('click', () => {
    ...
    if (!name || !target) { toast('Variabel identitas spesifik diperlukan', 'error'); return; }
    const goals = DB.getGoals();
    goals.push({ id: DB.generateId(), name, target, current, deadline });
    DB.saveGoals(goals);
    ['goalName','goalTarget','goalCurrent','goalDeadline'].forEach(id => { const el = document.getElementById(id); if (el) el.value = ''; });
    renderGoalsPage(); renderDashGoals();
    toast(`Parameter sasaran baru teregistrasi`, 'success');
  });
  ...
  document.getElementById('saveRecurring')?.addEventListener('click', () => {
    ...
    if (!name || !amount) { toast('Atribut data kueri diwajibkan', 'error'); return; }
    const recurrings = DB.getRecurring();
    ...
    renderRecurringPage();
    toast(`Looping tugas otomatis terkonfigurasi`, 'success');
  });
  ...
  document.getElementById('addNwAsset')?.addEventListener('click', () => {
    ...
    if (!name || !value) { toast('Deteksi kekurangan masukan data', 'error'); return; }
    const assets = DB.getNWAssets();
    ...
    saveNWSnapshot(); renderNetWorthPage();
    toast('Elemen aset didefinisikan', 'success');
  });
  document.getElementById('addNwDebt')?.addEventListener('click', () => {
    ...
    saveNWSnapshot(); renderNetWorthPage();
    toast('Elemen liabilitas didefinisikan', 'success');
  });
  ...
  document.getElementById('addSplitPerson')?.addEventListener('click', () => {
    ...
    if (!name) { toast('Deskripsi subjek nihil', 'error'); return; }
  ...
  document.getElementById('aiCustomSend')?.addEventListener('click', () => {
    ...
    if (!prompt) { toast('Kueri terminal kosong', 'warning'); return; }
  ...
  document.getElementById('saveWallets')?.addEventListener('click', () => {
    const rows = document.querySelectorAll('#walletsSettingsList .wallet-setting-item');
    ...
    rows.forEach(row => {
      ...
      wallets[key] = balance;
      meta[key] = { name, type, target: 0 };
    });
    DB.saveWallets(wallets); DB.saveWalletMeta(meta);
    toast('Matriks akun tersinkronisasi', 'success');
  ...
  document.getElementById('saveSystemSettings')?.addEventListener('click', () => {
    ...
    toast('Spesifikasi limit tersimpan', 'success');
    renderDashboard();
  });

  document.getElementById('savePersonalSettings')?.addEventListener('click', () => {
    const s = DB.getSettings();
    DB.saveSettings({
      ...s,
      name: document.getElementById('settingName')?.value.trim() || ''
    });
    toast('Registri kredensial ditimpa', 'success');
    renderDashboard();
  });
  ...
  document.getElementById('addObligation')?.addEventListener('click', () => {
    ...
    if (!name) { toast('Basis data string gagal divalidasi', 'error'); return; }
    if (!amount) { toast('Basis data nilai gagal divalidasi', 'error'); return; }
    const obs = DB.getObligations();
    ...
    renderObligationsSettings(); renderDashboard();
    toast(`Node liabilitas tetap diinisialisasi`, 'success');
  });
  ...
  document.getElementById('resetMonth')?.addEventListener('click', () => {
    showConfirm('Modus Transisi Periode', 'Sekuens operasi melancarkan penghapusan status konfirmasi pada blok kewajiban bulanan. Data master historis dipertahankan murni. Lanjutkan?', () => {
      ...
      renderObligationsSettings(); renderDashboard();
      toast('Transisi siklus sukses diaplikasikan', 'success');
    });
  });

  document.getElementById('clearAllData')?.addEventListener('click', () => {
    showConfirm('Operasi Pembersihan Tingkat Tinggi', 'Aksi ini menjadwalkan perombakan total pada struktur database lokal. Pemusnahan record mustahil dibalikkan tanpa backup fisik.', () => {
      ...
      toast('Eksekusi pembersihan absolut terproses', 'info');
      navigateTo('dashboard');
    });
  });
